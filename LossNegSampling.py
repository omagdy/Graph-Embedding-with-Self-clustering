...
class LossNegSampling(nn.Module):
    ...
    def __init__(self, num_nodes, emb_dim, nb_labels):
        ...
        self.embedding_com.weight.data.copy_(
            self.embedding_u(torch.LongTensor(np.random.randint(0, num_nodes, size=nb_labels))))
    ...
    def forward(self, u_node, v_node, negative_nodes):
        ...
        clustering_cost = (dist.min(dim=1)[0] ** 2).mean()

        total_loss = loss1 + clustering_cost

        cluster_label = self.cluster_labels(self.embedding_u(u_node), self.embedding_com.weight)
