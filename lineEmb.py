...
class lineEmb():
    ...
    def train(self):
        ...
        model = LossNegSampling(len(set(self.all_nodes)), self.emb_size)
        ...
        for epoch in range(self.epoch):
            ...
            t2 = time.time()
            print(
            self.name, 'loss: %0.3f ' % np.mean(final_losses), 'Epoch time: ', '%0.4f' % (t2 - t1), 'dimension size:',
            self.emb_size, ' alpha: ', self.alpha)
